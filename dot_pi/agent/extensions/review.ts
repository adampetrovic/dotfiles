...
let detectedVcs: VcsType | null = null;

async function detectVcs(pi: ExtensionAPI): Promise<VcsType> {
	if (detectedVcs) return detectedVcs;
...
async function getMergeBase(
	...
	if (vcs === "jj") {
		// jj doesn't have merge-base; we don't need it because jj diff --from works directly
		return null;
	}
...
/**
 * Get list of local branches/bookmarks
 */
async function getLocalBranches(pi: ExtensionAPI): Promise<string[]> {
	const vcs = await detectVcs(pi);

	if (vcs === "jj") {
		const { stdout, code } = await pi.exec("jj", ["bookmark", "list", "--template", 'name ++ "\\n"']);
		if (code !== 0) return [];
		return stdout
			.trim()
			.split("\n")
 ...
 * For jj, checks if the working copy has any changes.
 */
async function hasUncommittedChanges(pi: ExtensionAPI): Promise<boolean> {
	...
	if (vcs === "jj") {
		...
		const { stdout, code } = await pi.exec("jj", ["diff", "--stat"]);
 ...
 * For jj, working copy changes are always committed so this is generally safe.
 * We still check for conflicts which would prevent operations.
 */
async function hasPendingChanges(pi: ExtensionAPI): Promise<boolean> {
	const vcs = await detectVcs(pi);

	if (vcs === "jj") {
		// In jj, working copy is always a commit. Check for conflicts instead.
		const { stdout, code } = await pi.exec("jj", ["status"]);
		if (code !== 0) return false;
		// If there are conflicts, that's a problem
...
async function getCurrentBranch(pi: ExtensionAPI): Promise<string | null> {
	...
	if (vcs === "jj") {
		// Get bookmarks pointing to the current change
		const { stdout, code } = await pi.exec("jj", [
			"log", "-r", "@", "--no-graph",
		...
		if (code === 0 && stdout.trim()) {
			...
			const first = stdout.trim().split(",")[0];
			return first || null;
		}
		return null;
	}
...
/**
 * Get the default branch/bookmark (main or master)
 */
async function getDefaultBranch(pi: ExtensionAPI): Promise<string> {
	const vcs = await detectVcs(pi);

	if (vcs === "jj") {
		// Check if main or master bookmark exists
...
/**
 * Build the review prompt based on target and detected VCS
 */
async function buildReviewPrompt(pi: ExtensionAPI, target: ReviewTarget): Promise<string> {
	...
	switch (target.type) {
		case "uncommitted":
			if (vcs === "jj") {
				return "Review the current working copy changes. Run `jj diff` to see the changes in the working copy commit. Provide prioritized findings.";
			}
			return UNCOMMITTED_PROMPT;

		case "baseBranch": {
			if (vcs === "jj") {
				return `Review the code changes against the bookmark '${target.branch}'. Run \`jj diff --from '${target.branch}' --to @\` to inspect the changes relative to ${target.branch}. Provide prioritized, actionable findings.`;
			}
		...
		case "commit":
			if (vcs === "jj") {
				if (target.title) {
					return `Review the code changes introduced by change ${target.sha} ("${target.title}"). Run \`jj diff -r ${target.sha}\` to see the changes. Provide prioritized, actionable findings.`;
				}
				return `Review the code changes introduced by change ${target.sha}. Run \`jj diff -r ${target.sha}\` to see the changes. Provide prioritized, actionable findings.`;
			}
			if (target.title) {
		...
		case "pullRequest": {
			if (vcs === "jj") {
				return `Review pull request #${target.prNumber} ("${target.title}") against the base bookmark '${target.baseBranch}'. Run \`jj diff --from '${target.baseBranch}' --to @\` to inspect the changes that would be merged. Provide prioritized, actionable findings.`;
			}
...
function getUserFacingHint(target: ReviewTarget): string {
	const isJj = detectedVcs === "jj";
	switch (target.type) {
		case "uncommitted":
			return isJj ? "working copy changes" : "current changes";
		case "baseBranch":
			return `changes against '${target.branch}'`;
		case "commit": {
			const shortSha = target.sha.slice(0, 7);
			const label = isJj ? "change" : "commit";
			return target.title ? `${label} ${shortSha}: ${target.title}` : `${label} ${shortSha}`;
		}
		case "custom":
...
// Labels are dynamically adjusted based on detected VCS in showReviewSelector
const REVIEW_PRESETS = [
	{ value: "uncommitted", label: "Review uncommitted changes", jjLabel: "Review working copy changes", description: "" },
	{ value: "baseBranch", label: "Review against a base branch", jjLabel: "Review against a bookmark", description: "(local)" },
	{ value: "commit", label: "Review a commit", jjLabel: "Review a change", description: "" },
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	async function showReviewSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		const items: SelectItem[] = REVIEW_PRESETS.map((preset) => ({
			value: preset.value,
			label: (isJj && (preset as any).jjLabel) ? (preset as any).jjLabel : preset.label,
	...
	/**
	 * Show branch/bookmark selector for base branch review
	 */
	async function showBranchSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		// Never offer the current branch as a base (reviewing against itself is meaningless).
		const candidateBranches = currentBranch ? branches.filter((b) => b !== currentBranch) : branches;
		const branchLabel = isJj ? "bookmarks" : "branches";

		if (candidateBranches.length === 0) {
			ctx.ui.notify(
				currentBranch ? `No other ${branchLabel} found (current: ${currentBranch})` : `No ${branchLabel} found`,
				"error",
			);
			return null;
		}
		...
		const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
			const container = new Container();
			container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
			container.addChild(new Text(theme.fg("accent", theme.bold(isJj ? "Select base bookmark" : "Select base branch"))));
	...
	async function showCommitSelector(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		...
		const result = await ctx.ui.custom<{ sha: string; title: string } | null>((tui, theme, _kb, done) => {
			const container = new Container();
			container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
			container.addChild(new Text(theme.fg("accent", theme.bold(detectedVcs === "jj" ? "Select change to review" : "Select commit to review"))));
	...
	async function showPrInput(ctx: ExtensionContext): Promise<ReviewTarget | null> {
		// First check for pending changes that would prevent operations
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			detectedVcs = null; // Reset detection for each review
			const vcs = await detectVcs(pi);
			if (vcs === "git") {
