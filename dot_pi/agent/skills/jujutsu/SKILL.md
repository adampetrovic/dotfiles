...
For **hunk-level** (sub-file) operations without an interactive TUI, load and follow the separate `jjc` skill. It is the authoritative guide for inspecting, picking, dropping, and folding individual hunks.
...
3. **Avoid interactive modes**: Do not run bare `jj split`, `jj split -i`, `jj squash -i`, or bare `jj resolve` in agent sessions unless the user explicitly wants an interactive tool. For hunk-level work, load and follow the separate `jjc` skill.
...
13. **Programmatic commit splitting**: For hunk-level splitting or selective fold/drop operations, load and follow the separate `jjc` skill; never use `jj split -i` in agent sessions.
